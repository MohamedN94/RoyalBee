...
class InformationController extends Controller
{
    ...
    public function show(DataTables $dataTables, Request $request)
    {
        ...
        $model = Information::orderBy('created_at', 'desc');
        return $dataTables->eloquent($model)->addIndexColumn()
        ->editColumn('id', function (Information $Information) {
                return $Information->id ?? '-';
            })->editColumn('phone', function (Information $Information) {
                return $Information->phone ?? '-';
            })->editColumn('about', function (Information $Information) {
                return App::getLocale() == 'ar' ? ($Information->about_ar ?? '-') : ($Information->about_en ?? '-');
            })->editColumn('address', function (Information $Information) {
                return App::getLocale() == 'ar' ? ($Information->address_ar ?? '-') : ($Information->address_en ?? '-');
            })->editColumn('created_at', function (Information $Information) {
                return $Information->created_at->format('d-m-Y h:i A') ?? '-';
            })->addColumn('action', function (Information $Information) {
                return view('dashboard.information.buttons', compact('Information'));
            })
            ->rawColumns(['action'])
            ->startsWithSearch()
            ->make(true);
