...
class BestSellerProduct extends Component
{
    ...
    {
        $this->products = Product::select(['id', 'name_ar', 'name_en', 'description_ar', 'description_en', 'slug', 'image', 'price', 'discount_price', 'review'])
            ->where('best_seller', 1)
            ->where('is_visible', 1)
            ->with('photos')
            ->get();
    ...
    {
        $cart = Cart::content();
        return view('livewire.best-seller-products', compact('cart'));
    }
