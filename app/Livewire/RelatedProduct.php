...
class RelatedProduct extends Component
{
    ...
    public function mount($productId)
    {
        ...
        $this->relatedProducts = Product::where('category_id', $product->category_id)
            ->where('is_visible', 1)
            ->where('id', '!=', $productId)
            ->select(['id', 'name_ar', 'name_en', 'description_ar', 'description_en', 'slug', 'image', 'price', 'discount_price', 'review', 'best_seller'])
            ->with('photos')
            ->get();
    ...
    {
        $cart = Cart::content();
        return view('livewire.related-product', compact('cart'));
    }
